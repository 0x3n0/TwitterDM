...
class TwitterBot {
    constructor(props) {
        this.T = new Twit({
            consumer_key: props.env.TWITTER_API_KEY,
            consumer_secret: props.env.TWITTER_API_SECRET,
            access_token: props.env.TWITTER_ACCESS_TOKEN_KEY,
    ...
    getUnnecessaryMessages = (receivedMessages, trigger) => {
        return receivedMessages.filter(msg => {
            const message = msg.message_create.message_data.text; // 'Halo nama gw Eno coy!'
            const words = this.getEachWord(message); // ['Halo', 'nama', 'gw', 'Eno', 'coy!']
